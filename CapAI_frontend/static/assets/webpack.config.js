...
function addPreamble(entry) {
  return path.join(APP_DIR, entry);
}
...
const config = {
  ...
  entry: {
    capai: addPreamble('/src/index.jsx'),
  },
  output,
  optimization: {
    splitChunks: {
  ...
  devServer: {
    ...
    // and proxy everything else to capaiPort backend
    proxy: {
      context: () => true,
      '/': `http://localhost:${capaiPort}`,
      target: `http://localhost:${capaiPort}`,
    },
